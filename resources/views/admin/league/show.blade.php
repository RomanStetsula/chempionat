...
<div class="show-league">
  <div class="links col-xs-10">
    <a  href="{{ URL::to('admin-leagues')}}">Повернутсь до огляду ліг <i class="fa fa-reply" aria-hidden="true"></i></a>
  </div>
  <div class="links col-xs-2">
      <a  href="{{ URL::to('admin-calendar/'.$league->id)}}">Календар <i class="fa fa-list" aria-hidden="true"></i></a>
  </div>
  <div class ="col-xs-12">
  <table class="table table-striped table-bordered">  
      <div class="table-title"><h5><strong>{{$league->league_name}}</strong></h5></div>
    <thead>  
      <tr>
          <th>Поз.</th>
          <th>Команда</th>
    ...
    <tbody>
      @foreach($league_teams as $key => $league_team)
      <tr>
          <td>{{$key+1}}</td>
          ...
          <td >
              {{ Form::open(['route' => ['admin-league.destroy', $league_team->id], 'method' => 'DELETE']) }}
  ...
  {!! Form::open(['url'=>'admin-league']) !!}
  <div class="form-group">
      {!! Form::select('teams_add[]', ['Виберіть команди...'=>$all_teams], null, ['multiple', 'size' => 10]) !!}
